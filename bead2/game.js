...
function Field(p, type, rot, scl, regGoalNum) {
	this.pos  = p    || vec2();
	this.type = type || 0;
	this.rot  = rot  || 0;
	...
	if (su_mode && regGoalNum != null && (type == FieldT.MirrorGoal || type == FieldT.Goal)) {
		add_targets(1);
	}
};
...
var SuModeDrawer = {
	...
	init: function() {
		for (var i=1;i!=9;i++) {
			this.fields.push(new Field(this.getPosFromIndex(i),FieldT.getFromId(i),0,0.95));
		}
	},
	getPosFromIndex: function(i) {
		var s = Field_size.mul(Field_scale);
		return vec2(this.offsetx,s.y * 3).add(s.mul(vec2(1,i))).add(s.mul(vec2(-0.5,-0.5)));
	},
	findIdFromPos: function(p) {
		var s = Field_size.mul(Field_scale);
		
		if (p.x > this.offsetx && p.x < this.offsetx + s.x && 
			p.y > s.y * 3 && p.y < s.y * 3 + s.y * this.fields.length)
		{
			return Math.floor((p.y - s.y * 3) / s.y);
		}
		
		return -1;
	},
	draw: function() {
		drawGrid(vec2(1,this.fields.length),
				 Field_size.mul(Field_scale),
				 vec2(this.offsetx,Field_size.mul(Field_scale).y * 3),
	...
	handlePress: function(p) {
		if (!isDraggingField() && su_mode)
		{
			var i = this.findIdFromPos(p);
			if (i != -1)
			{
				dragField = new Field(this.fields[i].pos,this.fields[i].type,this.fields[i].rot,1,true);
				dragField.setPos(p);		
			}
		}
	}
...
PlayButtonTemplate.prototype = {
	...
	contains: function(p) {
		...
		return (p.sub(this.offset).length() < this.siz_transition.get());
	},
	handleHover: function(p) {
		var ins = this.contains(p);
		
		if (ins && !this.mouse_on) {
			
			this.mouse_on = true;
			this.updateState(true,false);
			canvas.style.cursor = "pointer";
			
		} else if (!ins && this.mouse_on) {
...
BackButton.offset = PlayButton.offset.add(vec2(-Field_size.mul(Field_scale).x * 2,0));
...
var UploadButton = new PlayButtonTemplate([40,100,100,0,1],[45,180,130,100,3],[43,150,120,50,2]);

UploadButton.disabled = (GameData[0][2] == 0);
UploadButton.offset = PlayButton.offset.add(vec2(0,Field_size.mul(Field_scale).y));
UploadButton.contains = function(p) {
	
	if (this.disabled) return false;
	
	var size = this.siz_transition.get();
	var delta = p.sub(this.offset);
	
	if (delta.length() < size)
	{
		return Math.abs(delta.y) < size / 2;
	}
	
	return false;
};
UploadButton.draw = function() {
	var size  = this.siz_transition.get();
	var red   = Math.floor(this.red_transition.get());
	var green = Math.floor(this.gre_transition.get());
	var blue  = Math.floor(this.blu_transition.get());
	...
	if (this.disabled) {
		var lum = 0.21 * red + 0.72 * green + 0.07 * blue;
	...
	var wText = ctx.measureText("Feltöltés").width;
	
	ctx.fillText("Feltöltés",this.offset.x - wText / 2,this.offset.y + 10);
	
	ctx.shadowBlur = 0;
};

var upload_from = document.getElementById("upload_from");
var ori_canvasH = canvas.height;

UploadButton.onRelease = function() {
	canvasHeight.set(canvas.height,0,0.5);
	
	upload_from.parentNode.removeChild(upload_from);
	document.body.appendChild(upload_from);
	upload_from.style.display = "block";
	
	var cancel_btn = document.getElementById("cancel_btn");
	
	unreg_input_handlers();
	
	cancel_btn.onclick = function() {
		canvasHeight.set(0,ori_canvasH,0.5);
		reg_input_handlers();
	};
	...
	upload_btn.onclick = function() {
		var creator = document.getElementById("in_maker").value;
		var map_name = document.getElementById("in_map_name").value;
		var difficulty = document.getElementById("in_dif").value;
		var map_code = get_map_code(" ");
		...
		xmlhttp.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				...
				canvasHeight.set(0,ori_canvasH,0.01);
				upload_from.style.display = "none";
				gen_table();
				update_table();
			}
		};
		...
		xmlhttp.send( "username="   + php_username + 
...
ResetButton.offset = PlayButton.offset.add(vec2(Field_size.mul(Field_scale).x * 2,0));
...
ResetButton.onRelease = function() {
	...
	for (f of drawer_to_map) {
		...
		if (f.init_map_place.x == -1) {
			DeletedItemsHandler.add(f);
		} else {
...
var handleDragDrop = function(p) {
	...
	else
	{
		if (su_mode && p.x < canvas.width - Drawer.width - 10) {
			DeletedItemsHandler.add(dragField);
			dragField = null;
		} else {
			Drawer.addItem(p,dragField);
			dragField = null;
...
var DeletedItemsHandler = {
	...
	draw: function() {
		...
		for (f of this.fields) {
			...
			if (!f.scl_transition.finished()) {
				fieldscpy.push(f);
			} else if (su_mode && (f.type == FieldT.MirrorGoal || f.type == FieldT.Goal)) {
				add_targets(-1);
			}
		}
		
		this.fields = fieldscpy;
	},
	add: function(f) {
		this.fields.push(f);
		f.setDrawScale(0);
		
		var v = vec2(canvas.width/2,canvas.height/2).sub(f.pos);
		
		f.setPos(f.pos.add(v.sgn().mul(20)));
	}
};
...
var canvasHeight = new Transition(canvas.height,canvas.height,0,2);

/* * * * * * * * * * RENDERING METHOD * * * * * * * * * */
render = function () {
	
	if (!canvasHeight.finished() || canvasHeight.get() != canvas.height)
	{
		canvas.height = canvasHeight.get();
		
		var opc = 1 - canvas.height / ori_canvasH;
		
		if (opc > 0.25)
		{
			opc = (opc - 0.25) / (1 - 0.25);
			upload_from.style.opacity = opc;
			upload_from.style.display = "";
		}
		else
		{
			upload_from.style.display = "none";
...
var onMouseLeftPress = function(p) {
	...
	else
	{
		...
		if (su_mode) UploadButton.handlePress(p);
		
		if (isModyfingAllowed())
		{
			SuModeDrawer.handlePress(p);
...
var get_map_code = function(eol) {
	var text = "";
	
	var max_x = Map.fields.length - 1;
	var max_y = Map.fields[0].length - 1;
		...
		for (var x = 0;x < Map.size.x;x++) {
			if (x <= max_x && y <= max_y) {
				...
				if (field != null) {
					...
					text = text + field.type.id + "," + field.rot + ",1," + (Map.fields[x][y].rotfix ? 1 : 0) + ";EOL";
				} else {
					nullDb++;
				}
			}
	...
	return text.split("EOL").join(eol);
};

var reg_input_handlers = function() {
	...
	window.onkeydown = function(e) {
		...
		if (e.keyCode == 76) // l
		{
			if (isModyfingAllowed())
			{
				if (!isDraggingField() && su_mode)
				{
					var i = Map.findIdFromPos(lastMouseP); 
					if (i.x != -1)
					{
						var f = Map.fields[i.x][i.y];
			...
			(e.keyCode >= 96 && e.keyCode <= 105))
		{
			...
			if (isModyfingAllowed()) {
				if (!isDraggingField()) {
					...
					if (su_mode) {
						dragField = new Field(SuModeDrawer.getPosFromIndex(n),FieldT.getFromId(n),0,1.0,true);
	...
	window.onkeyup = function(e) {
		...
		if (e.keyCode == 84) // t
		{
			...
			if (su_mode) UploadButton.handleRelease(lastMouseP);
		}
		
	};
};

var unreg_input_handlers = function() {
...
canvas.onmouseup = function(e) {
	if (e.button == 0)
	{
		...
		if (su_mode) UploadButton.handleRelease(p);
...
canvas.onmousemove = function(e) {
	...
	if (!isDraggingField()) {
		...
		if (su_mode) UploadButton.handleHover(p);
	}
